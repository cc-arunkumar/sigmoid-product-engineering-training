//now commenting this out as service will get the data and handle it 
...
exports.getAllProducts = async (req, res,next) => {
  ...
  try{
    //only mongodb version
    // const products=await Product.find()
    //integrated servicces file to have both mongodb and sql
    const products=await productService.getAllProducts();
  //return successResponse(res, "Products fetched successfully", products);
  return new AppResponse({ data: products,message:"All products fetched successfully"}).send(res);
  }catch(error){
    // return next({
    //         status:500,
...
//without success response
  ...
  return new AppResponse({ data: product,message:"Product fetched succesfully" }).send(res);
  }}catch(error){
//  return next({
//             status:500,
...
exports.createProduct = async (req, res,next) => {
  try{
    //commented out line 76 as we are going to pass body of request directly 
...
exports.updateProduct = async (req, res,next) => {
  ...
  //   return next(new AppError("Product not found", 404));
  // }
  //passing entire rew.body to our services file 
  // const { name, price, category, stock } = req.body;
  // product.name = name;
  // product.price = price;
