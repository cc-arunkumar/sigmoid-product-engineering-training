...
# asyncsession is used to improve performance by making use of threads concept where async ensures multiple users can hit same endpoint and it still works 
# conflicts of data can happen however due to thread context switching due to async session 
...
@router.post("/")
async def add_product(product:Product_Put_Post,db: AsyncSession=Depends(get_db)):
...
async def update_product(product_data:Product_Put_Post,product_id:int,db:AsyncSession = Depends(get_db)):
    updatedprod=await update(db,product_id,product_data,)
    if not updatedprod:
        raise HTTPException(status_code=404,detail="Product not found")
    return updatedprod

@router.patch("/{product_id}")
async def update_product_partial(product_data: Product_Patch,product_id:int,db:AsyncSession = Depends(get_db)):
    patchedprod=await patch_product(db,product_id,product_data)
    if not patchedprod:
        raise HTTPException(status_code=404,detail="Product not found")
    return patchedprod
...
async def delete_product(product_id:int,db:AsyncSession = Depends(get_db)):
    returnresult=await delete_product_by_id(db,product_id)
    if(returnresult):
