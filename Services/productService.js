...
const productSQL = require("../models/product.sql.js");
...
const getAllProducts = async () => {
    ...
    if (USE_MONGO) {
        mongoResult = await Product.find();
    }
...
const getProductById = async (id) => {
    ...
    if (USE_MONGO && isMongoId(id)) {
        mongoResult = await Product.findById(id);
    }
...
const createProduct = async (data) => {
    ...
    if (USE_MONGO) {
        mongoResult = await Product.create(data);
    }
...
const updateProduct = async (id, data) => {
    ...
    let sqlResult = null;
        if (USE_MONGO && isMongoId(id)) {
        mongoResult = await Product.findByIdAndUpdate(
            id,
            data,
...
const deleteProduct = async (id) => {
    ...
    if (USE_MONGO && isMongoId(id)) {
        mongoResult = await Product.findByIdAndDelete(id);
    }
